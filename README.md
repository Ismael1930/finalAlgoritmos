# Prestamos API - Sistema de Gestión de Préstamos Ismael Nuñez 100284129
...
Este proyecto implementa algoritmos matemáticos y financieros para el cálculo de préstamos:

- **Algoritmos aritméticos**: Se utilizan para el cálculo de cuotas, intereses y saldos pendientes.
- **Algoritmos de amortización**:
  - **Método francés (cuota fija)**: Utiliza la fórmula de anualidad para calcular cuotas constantes, donde el interés disminuye y el capital aumenta progresivamente.
  - **Método alemán (cuota variable)**: Calcula cuotas variables, amortizando el capital de forma constante y variando el interés según el saldo pendiente.
- **Encriptación de contraseñas**: Las contraseñas de los usuarios se encriptan usando bcrypt antes de almacenarse en la base de datos.

Estos algoritmos están implementados en el servicio de amortización (`src/loans/services/amortization.service.ts`) y en el módulo de usuarios (`src/users/users.service.ts`).
